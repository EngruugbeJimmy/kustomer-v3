  ...
  try {
    ...
    if (plan.customerLimit > 0 && current >= plan.customerLimit)
      return res.status(402).json({
        error: "Customer limit reached for your plan. Upgrade to add more.",
        limit: plan.customerLimit, current
      });
    const { name, phone, notes } = req.body;
    const customer = await Customer.create({
      owner: req.user._id, name, phone: phone.replace(/[\s\-()]/g, ""), notes
    });
    res.status(201).json({ customer });
  } catch (e) {
    if (e.code === 11000) return res.status(400).json({ error: "Phone number already saved" });
