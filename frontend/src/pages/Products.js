...
function ProductModal({ product, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-10 overflow-y-auto max-h-[92vh]" onClick={e=>e.stopPropagation()}>
        <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-5"/>
        <h2 className="font-display text-xl font-bold mb-4">{editing?"Edit Product":"Add New Product"}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex gap-3"><button type="button" onClick={onClose} className="btn-outline">Cancel</button>
            <button type="submit" className="btn-green" disabled={loading}>{loading?"⏳ Saving...":editing?"Save Changes":"Add Product"}</button></div>
        </form>
      </div>
    </div>
  );
