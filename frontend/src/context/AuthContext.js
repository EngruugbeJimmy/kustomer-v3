...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(()=>{ const t=localStorage.getItem("kustomer_token"); const u=localStorage.getItem("kustomer_user"); if(t&&u) setUser(JSON.parse(u)); setLoading(false); },[]);
  const save   = (token,user) => { localStorage.setItem("kustomer_token",token); localStorage.setItem("kustomer_user",JSON.stringify(user)); setUser(user); };
  const login  = async(e,p)   => { const {data}=await api.post("/auth/login",{email:e,password:p}); save(data.token,data.user); return data.user; };
